...
export async function PUT(
  ...
  try {
    ...
    const existing = await selectQuery<WarrantyStatus>(
      `SELECT id FROM claim_statuses WHERE id = ? AND is_deleted = FALSE`,
      [id]
    );
...
export async function DELETE(
  ...
  try {
    ...
    await mutationQuery(
      `UPDATE claim_statuses SET is_deleted = TRUE WHERE id = ?`,
      [id]
    );
